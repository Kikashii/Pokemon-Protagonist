...
def drawGameOver(data,hc):  # draws gameover
    font = pygame.font.Font("Assets/pokemon_pixel_font.ttf", 40)
    ...
    score = font.render(("Score : %d" % data.score), True, (0, 0, 0))
    data.screen.blit(score, (900, 512))
    restart = font.render("Restart?", True, (0, 0, 0))
    data.screen.blit(restart, (701, 572))
    high = font.render(("Highscore : %d" % int(hc)), True, (0, 0, 0))
    data.screen.blit(high, (900, 572))


def redrawAll():  # redraws all draw functions
    if data.gameOver:
        hc=highscore(data)
        drawGameOver(data,hc)
    elif data.intro:
        drawIntro(data)
    else:
        drawAllBullets(data)
